      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === aiMessageId
            ? {
                ...msg,
                content: "I'm sorry, I couldn't connect. Try again later.",
              }
            : msg
        )
      );
  ...
  return (
    <>
      <Button
        ...
        className={`fixed bottom-6 right-6 h-14 px-4 rounded-full shadow-lg w-fit transition-all duration-300 hover:scale-110 z-50 flex items-center gap-2 ${
          isOpen ? "opacity-0 pointer-events-none" : "opacity-100"
        }`}
        size="icon"
      >
        <MessageCircle className="h-6 w-6" />
        <span>Ask Colora</span>
      </Button>
