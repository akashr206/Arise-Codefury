...
export default function StoryDialog({ reel, isOpen, onClose }) {
    ...
    useEffect(() => {
        if (isOpen && reel && reel.products && reel.products.length > 0) {
            fetchLinkedProducts();
        }
    }, [isOpen, reel]);
    ...
    const fetchLinkedProducts = async () => {
        try {
            const productPromises = reel.products.map(async (productId) => {
                                ...
                                <div className="mb-3">
                                    ...
                                    <div className="flex gap-2 overflow-x-auto pb-1">
                                        {linkedProducts.map((product) => (
                                            <div
                                                key={product._id}
                                                className="flex-shrink-0 bg-white/10 backdrop-blur-sm rounded-lg p-2 min-w-[160px] sm:min-w-[180px] border border-white/20 hover:bg-white/20 transition-colors duration-200 cursor-pointer"
                                                onClick={() => handleProductClick(product)}
                                            >
                                                <div className="flex items-center gap-2">
                                                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg overflow-hidden bg-white/20">
                                                        <img
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        {product.medium && (
                                                            <Badge
                                                                variant="secondary"
                                                                className="mt-1 text-[10px] sm:text-xs bg-white/20 text-white border-white/30"
                                                            >
                                                                {product.medium}
                                                            </Badge>
                                                        )}
